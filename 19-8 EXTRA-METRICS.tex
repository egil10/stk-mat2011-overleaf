...
This subsection defines the distributional, tail-risk, and extra risk-adjusted metrics that are reported in the per-pair tearsheets but kept out of Section~\ref{sec:backtest} so the body could stay focused on the three core measures (volatility, Sharpe, max drawdown). Notation follows Section~\ref{sec:metrics}: $R_t$ is the strategy return, $\mu_R$ and $\sigma_R$ its mean and standard deviation, $V_t$ the portfolio value with running peak $V_t^{\star}$, and $R_f = 0$ the benchmark.
