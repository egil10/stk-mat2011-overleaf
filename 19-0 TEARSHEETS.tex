...
The five blocks in the tables below read as follows. The Return block reports the realised cumulative PnL in basis points over the episode (Total Return) and its annualised counterpart (Annual Return, computed as $\text{bps}\times T_{\text{ann}}/T_{\text{episode}}$). The Risk block reports annualised volatility $\sigma_{\mathrm{ann}}$ of \eqref{eq:coremetrics}, the maximum drawdown $\mathrm{MDD}$ and its duration in bars, the Ulcer index $\mathrm{UI}$ of \eqref{eq:ulcer}, and the $95\%$ Value-at-Risk and Conditional VaR of \eqref{eq:varcvar}. The Risk-adj.\ block reports the Sharpe ratio of \eqref{eq:coremetrics} and the Sortino and Calmar ratios of \eqref{eq:extraratios}, together with the trade-level Profit Factor (sum of winning trades divided by absolute sum of losing trades), Payoff Ratio (average win divided by average loss), and a Tail Ratio (the ratio of the $95$th to the absolute $5$th percentile of returns). The Trading block reports the number of round-trip trades, the win rate, the market exposure (fraction of bars with a non-zero position), and the average winning and losing trade in basis points. The Distr.\ block reports the per-bar skewness $\mathcal{S}$ and kurtosis $\mathcal{K}$ of \eqref{eq:distshape}, both of which are large in magnitude under the conditionally heteroskedastic, heavy-tailed return distribution discussed in Section~\ref{sec:result-pairdiag}.
