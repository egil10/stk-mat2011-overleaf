...
Every position change in leg $i$ pays the half-spread $\mathrm{HS}_t^{\,i}$ from \eqref{eq:midprice} plus a small fixed commission \parencite{aldridge2013hft}, so a round-trip on one pair-position costs roughly $\mathrm{HS}^{A} + \hat\beta\,\mathrm{HS}^{B}$ basis points. Strategies are evaluated walk-forward: the model parameters $\Theta$ and the hyperparameters $(W_\beta, W_z, z_q, z_x, z_v, \delta)$ are fitted on a training segment, applied to a disjoint out-of-sample test segment, and then the window is slid forward \parencite{lopezdeprado2018advances}.

\paragraph{Core performance metrics.}\label{sec:metrics}
We report three core metrics in the body: annualised volatility, the Sharpe ratio, and the maximum drawdown. A fuller set (skewness, excess kurtosis, VaR, CVaR, Sortino, Calmar, Ulcer index) is collected in Appendix~\ref{sec:apx-extra-metrics} and reported in the per-pair tearsheets. Let $R_t$ be the strategy return at bar $t$, $\mu_R = \mathbb{E}[R]$, $\sigma_R = \sqrt{\mathrm{Var}(R)}$, $V_t$ the portfolio value with running peak $V_t^{\star} = \max_{s \leq t} V_s$, and $R_f$ a benchmark risk-free rate (set to zero). The three core metrics are
